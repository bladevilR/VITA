...
def normalize_text(value: Any) -> Any:
    ...
    for raw_name, standard_name in LINE_NAME_MAP.items():
        normalized = normalized.replace(raw_name, standard_name)
    normalized = re.sub(r"[A-Za-z]+", lambda match: match.group(0).upper(), normalized)
    return normalized


SPECIALTY_SYNONYMS = {
    "ISCS": ["ISCS", "综合监控", "综合监控系统", "综合监控设备", "IBP", "IBP盘"],
    "屏蔽门": ["屏蔽门", "站台门", "PSD", "屏蔽门设备", "屏蔽门控制器"],
    "AFC": ["AFC", "AFC设备", "闸机", "自动售检票", "售检票", "TVM", "AGM", "BOM", "票务设备"],
    "BAS": ["BAS", "BAS设备", "环境监控", "环控系统", "环境与设备监控"],
    "FAS": ["FAS", "FAS设备", "火灾报警", "消防系统", "火灾自动报警"],
    "门禁": ["门禁", "门禁设备", "门禁系统", "通道门"],
    "电扶梯": ["电扶梯", "扶梯", "自动扶梯", "电扶梯设备"],
    "给排水": ["给排水", "给排水设备", "给水排水"],
