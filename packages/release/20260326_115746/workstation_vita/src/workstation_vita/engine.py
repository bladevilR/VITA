...
def _prepare_case_pool(layer_outputs: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    combined = combined.drop_duplicates(subset=["TICKETID"], keep="first").drop(columns="__bucket_order")
    return combined.reset_index(drop=True)
...
class WorkstationEngine:
    ...
    def process_query(self, user_query: str) -> dict[str, Any]:
        ...
        if intent == "responsibility":
            ...
            return {
                "intent": intent,
                "answer_markdown": payload.get("answer_markdown", "查询完成。"),
                "details": payload,
                "parsed": parsed,
            }
        ...
        answer = self.ai_client.call_llm_text(prompt=prompt, temperature=0.2, timeout=120)
