...
RESPONSIBILITY_KEYWORDS = ["谁负责", "归谁管", "哪个班组", "找谁", "联系谁", "责任班组"]
DIAGNOSIS_KEYWORDS = [
    "怎么办",
    "怎么处理",
    "怎么修",
...
STATION_INVALID_MARKERS = ("哪个站", "哪个车站", "故障", "最多", "本周", "本月", "今天", "近", "号线")
STATION_CONTEXT_WORDS = (
    "班组",
    "维修员",
    "值班员",
    "站务",
...
def _extract_station_name(query: str) -> str | None:
    ...
    for index, char in enumerate(query):
        ...
        cursor = index - 1
        while cursor >= 0 and len(prefix_chars) < 16:
            current = query[cursor]
            if current == "站" or not re.fullmatch(r"[\u4e00-\u9fa5]", current):
                break
            prefix_chars.append(current)
            cursor -= 1

        if not prefix_chars:
            continue

        prefix = normalize_text("".join(reversed(prefix_chars)))
        max_len = min(len(prefix), 8)
        for size in range(2, max_len + 1):
            candidates.append(f"{prefix[-size:]}站")
    ...
    for candidate in dict.fromkeys(candidates):
        ...
        if candidate in {"车站", "本站", "全站"}:
            continue
        if any(marker in candidate for marker in STATION_INVALID_MARKERS):
            continue

        score = len(candidate) * 10
        for word in STATION_CONTEXT_WORDS:
...
def _clean_fault_phenomenon(
    ...
    cleaned = re.sub(r"重启(?:后|以后)(?:还是|还|仍|仍然|依旧)([A-Za-z0-9\u4e00-\u9fa5]{1,12})", r"重启无效 \1", cleaned)
    for raw_text, normalized_text in FAULT_NORMALIZATIONS.items():
        cleaned = cleaned.replace(raw_text, normalized_text)
...
def fast_parse_local(user_query: str) -> dict[str, Any] | None:
    ...
    entities: dict[str, Any] = {
        ...
        "compare_dimension": compare_dimension if intent == "statistics" else None,
    }
...
"""
    try:
        result = ai_client.call_llm_json(
            prompt=prompt,
            temperature=0.0,
            timeout=45,
    ...
    if result.get("intent") == "statistics":
        result["entities"]["compare_dimension"] = result.get("compare_dimension")
