...
def _has_specific_device(entities: dict[str, Any]) -> bool:
    device = _safe_text(entities.get("device"))
    specialty = _safe_text(entities.get("specialty"))
    return bool(device and device != specialty)


def _should_skip_llm_refine(parsed: dict[str, Any], plan: dict[str, Any]) -> bool:
    entities = parsed.get("entities", {}) or {}
    query_type = parsed.get("query_type")
    station_name = _safe_text(entities.get("station_name"))
    specialty = _safe_text(entities.get("specialty"))
    has_specific_device = _has_specific_device(entities)
    has_fault = bool(plan.get("exact_fault_terms"))

    if query_type == "history" and station_name and (has_specific_device or specialty):
        return True

    if station_name and has_fault and (has_specific_device or specialty):
        return True
...
def _merge_plan(default_plan: dict[str, Any], llm_output: dict[str, Any]) -> dict[str, Any]:
    ...
    default_exact_terms = _dedupe([str(item) for item in default_plan.get("exact_fault_terms", []) if item])

    if isinstance(llm_output.get("exact_fault_terms"), list):
        raw_exact_terms = [str(item) for item in llm_output["exact_fault_terms"] if item]
        llm_exact_terms = _dedupe(raw_exact_terms)
        if llm_exact_terms:
            if default_exact_terms:
                expanded_terms = _dedupe(
                    [term for item in llm_exact_terms for term in split_fault_terms(item)]
                )
                if expanded_terms and all(term in default_exact_terms for term in expanded_terms):
                    plan["exact_fault_terms"] = [term for term in default_exact_terms if term in expanded_terms] or default_exact_terms
            else:
                plan["exact_fault_terms"] = llm_exact_terms[:4]
...
def build_retrieval_plan(user_query: str, parsed: dict[str, Any], ai_client: AIClient) -> dict[str, Any]:
    ...
    try:
        if _should_skip_llm_refine(parsed, plan):
            plan["planner_source"] = "heuristic_precise"
            return plan
        llm_output = ai_client.call_llm_json(
            prompt=_planner_prompt(user_query, parsed, plan),
            temperature=0.0,
            timeout=30,
